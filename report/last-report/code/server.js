...
server.on('connection', ws => {             // WSサーバ機能の実行 ～ クライアント接続時に対応
    ...
    const SendMessage = JSON.stringify({user: user, message: getmessage, from: 'myself'});
    const GetOtherMessage = JSON.stringify({user: user, message: getmessage, from: 'other'});

    ws.send(SendMessage.toString("utf8"));                       // 2) 送信クライアントのみにmessageの内容返信
    server.clients.forEach(client => {      // サーバ接続の全クライアントに対する処理の規定
        if (client !== ws)                  // 送信クライアントを除外する条件設定
            client.send(GetOtherMessage.toString("utf8"));               // 3) 接続クライアントへのmessageの内容送信
